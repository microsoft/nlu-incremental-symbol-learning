...
@TrainerBase.register("vanilla_calflow_parsing")
class VanillaCalflowTrainer(CalflowTrainer):
    ...
    def _update_validation_exact_match(self, pred_instances: List[Dict[str, numpy.ndarray]],
        ...
        pred_graphs = [CalFlowSequence.from_prediction(pred_inst['nodes']) 
                         for i, pred_inst in enumerate(pred_instances)]

        true_lispress_strs = [render_compact(parse_lispress(ts)) for ts in true_graphs]
        pred_lispress_strs = [render_compact(parse_lispress(ps)) for ps in pred_graphs]
        ...
        #exact_match_score, no_refer_score, __, __, __ = self.exact_match_metric.get_metric(reset=True)
        exact_match_score = self.exact_match_metric.get_metric(reset=True)
        self.model.exact_match_score = exact_match_score
        #self.model.no_refer_score = no_refer_score 
