...
class CalflowTrainer(Trainer):
    ...
    def _validation_loss(self) -> Tuple[float, int]:
        ...
        self._update_validation_exact_match(val_outputs, val_true_instances)

        return val_loss, batches_this_epoch

    def returns_batch_loss(self, batch_group: List[TensorDict], for_training: bool) -> torch.Tensor:
        """
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_group in train_generator_tqdm:
            ...
            if self._tensorboard.should_log_this_batch():
                ...
                if "interest_loss" in metrics.keys():
                    self._tensorboard.add_train_scalar("interest_loss/interest_loss_train", metrics["interest_loss"])
                    self._tensorboard.add_train_scalar("non_interest_loss/non_interest_loss_train", metrics["non_interest_loss"])
