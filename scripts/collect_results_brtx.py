...
def get_all_results(base_dir, data_dir, model_name, function = "FindManager", seeds = [12, 31, 64], splits = [5000, 10000, 20000, 50000, 100000, "max"], fxn_splits = [50, 100, 200, 500], test=False):
    if test:
        prefix = "test"
    else:
        prefix = "valid"
    ...
    for seed in seeds:
        for split in splits:
            for fxn_split in fxn_splits:
                ...
                # test for which partition it's on 
                for idx in range(1,7):
                    cbase_dir = re.sub("@", str(idx), base_dir)
                    candidate_dir = f"{cbase_dir}/{function}_{model_name}_{split}_{fxn_split}_{seed}_seed/" 
                    candidate_file = f"{cbase_dir}/{function}_{model_name}_{split}_{fxn_split}_{seed}_seed/translate_output/test_valid.tgt" 
                    if pathlib.Path(candidate_file).exists():
                        model_dir = pathlib.Path(candidate_dir) 
                        break 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-dir", type=str, help="path to dir where all models stored", default="/brtx/60@-nvme1/estengel/miso_models/")
    parser.add_argument("--model-name", type=str, help="name of the model trained", default="lstm_no_source")
    ...
    res_df = get_all_results(args.model_dir, data_dir, args.model_name, args.fxn, seeds = args.seeds, splits = args.splits, fxn_splits = args.fxn_splits, test=args.test) 
